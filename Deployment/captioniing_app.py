...
vocab = pickle.load(open(r"C:\Users\exact\Downloads\vocab.file", 'rb'))
...
def get_caption_model():
    ...
    caption_model((sample_x, sample_y), training=True)
    ...
    loaded_weights = np.load(
        r"C:\Users\exact\Downloads\image_captioning_transformer_wt.weights.h5", allow_pickle=True)
    for layer in caption_model.layers:
        if layer.name != 'loss' and 'optimizer' not in layer.name:
            layer_weights = loaded_weights[layer.name]
            layer.set_weights(layer_weights)

    return caption_model


captioning_model = get_caption_model()
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        result = generate_caption(image, captioning_model)
